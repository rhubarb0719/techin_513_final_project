...
def draw_stars(ax, flux, centroid, t):
    ...
    for i in range(n_stars):
        ...
        size    = (70 + flux * 180) * _STAR_BASE[i] * (0.6 + 0.4 * flicker)
...
class ParticleSystem:
    ...
    def render(self, ax, n_active, centroid):
        ...
        if xs:
            ...
            # Multi-layer glow for radial-like gradient (inner brighter, outer softer)
            glow_specs = [
                (4.0, 0.16, 5),
                (9.0, 0.09, 4),
                (16.0, 0.045, 3),
            ]
            for scale, alpha_mult, z in glow_specs:
                glow_colors = [gc(p['hue'], p['alpha'] * alpha_mult) for p in self.pool[:n_active]]
                ax.scatter(xs, ys, s=[s * scale for s in sizes], c=glow_colors,
                           marker='o', zorder=z, linewidths=0)
